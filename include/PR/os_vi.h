...
#define OS_VI_BURST(hsync_width, color_width, vsync_width, color_start) \
    (hsync_width | (color_width << 8) | (vsync_width << 16) | (color_start << 20))
#define OS_VI_WIDTH(v) v
#define OS_VI_VSYNC(v) v
#define OS_VI_HSYNC(duration, leap) (duration | (leap << 16))
#define OS_VI_LEAP(upper, lower) ((upper << 16) | lower)
#define OS_VI_START(start, end) ((start << 16) | end)

#define OS_VI_FTOFIX(val, i, f) ((u32)(val * (f32)(1 << f)) & ((1 << (i + f)) - 1))

#define OS_VI_F210(val) OS_VI_FTOFIX(val, 2, 10)
#define OS_VI_SCALE(scaleup, off) (OS_VI_F210((1.0f / (f32)scaleup)) | (OS_VI_F210((f32)off) << 16))

#define OS_VI_VCURRENT(v) v //seemingly unused
#define OS_VI_ORIGIN(v) v
#define OS_VI_VINTR(v) v
#define OS_VI_HSTART OS_VI_START

#if LIBULTRA_VERSION > OS_VER_F

#define OS_VI_BURST_CW_PAL 30
#define OS_VI_BURST_CS_PAL 69
#define OS_VI_HSYNC_LEAP_PAL 23
#define OS_VI_LEAP_LOWER_PAL 3181

#else

#define OS_VI_BURST_CW_PAL 35
#define OS_VI_BURST_CS_PAL 64
#define OS_VI_HSYNC_LEAP_PAL 21
#define OS_VI_LEAP_LOWER_PAL 3182
