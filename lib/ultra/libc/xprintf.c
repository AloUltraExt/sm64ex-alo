...
#define PUT(s, n)                                \
    if (0 < (n))                                 \
    {                                            \
        if ((arg = (*prout)(arg, (char*) s, n)) != NULL) \
            x.nchar += (n);                      \
        else                                     \
            return x.nchar;                      \
    }
...
static void _Putfld(_Pft *pf, va_list *pap, char_x code, char_x *ac);
...
static void _Putfld(_Pft *x, va_list *args, char_x type, char_x *buff) {
    ...
    switch (type) {
        ...
        case 'p':
            ...
            break;
        case 's':
            x->s = (u8 *) va_arg(*args, char_x *);
            x->n1 = strlen((char *)x->s);
