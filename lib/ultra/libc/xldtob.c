...
static void _Genld(_Pft* px, char_x code, u8* p, s16 nsig, s16 xexp);
...
void _Ldtob(_Pft* px, char_x code) {
    char_x buff[BUFF_LEN];
    char_x *p;
    f64 ldval;
    s16 err;
    s16 nsig;
    ...
    if (err > 0) {
        ...
        }
        {
            ...
            if (nsig > 0) {
                char_x drop = nsig < gen && '5' <= p[nsig] ? '9' : '0';
                int n;
...
void _Genld(_Pft* px, char_x code, u8* p, s16 nsig, s16 xexp) {
